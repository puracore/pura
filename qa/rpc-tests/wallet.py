...
class WalletTest (BitcoinTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s PURA too low! (Should be %s PURA)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s PURA too high! (Should be %s PURA)"%(str(fee), str(target_fee)))
        return curr_balance
    ...
    def run_test (self):
        ...
        # Send 210 PURA from 0 to 2 using sendtoaddress call.
        ...
        # Send 100 PURA with subtract fee from amount
        ...
        # Sendmany 100 PURA with subtract fee from amount
